...
-- The C engine tolerates these truncated inputs and fills missing data with
-- zeros. Rust intentionally rejects them; this Lua check documents the legacy
-- C/Lua behavior without treating it as the Rust contract.
local malformed_cases = {
    { "truncated-ff-only",       "\xff",     string.rep("\0", 8) },
    { "truncated-01-only",       "\x01",     string.rep("\0", 8) },
    { "truncated-ff-1byte",      "\xff\x42", "\x42" .. string.rep("\0", 7) },
    { "truncated-55-1byte",      "\x55\xaa", "\xaa" .. string.rep("\0", 7) },
    { "truncated-80-0byte",      "\x80",     string.rep("\0", 8) },
}
...
for _, c in ipairs(malformed_cases) do
    ...
    if unpacked == expected then
        ...
        mfail = mfail + 1
        io.stderr:write(string.format("MALFORMED [%s]: expected legacy bytes %s but got %s\n",
            c[1], hex_encode(expected),
            unpacked and hex_encode(unpacked) or "nil"))
    end
end
